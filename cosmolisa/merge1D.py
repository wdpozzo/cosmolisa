...
def evaluate_grid(density,x):
    sys.stderr.write("computing log posterior for %d grid points\n"%(len(x)))
    sample_args = ((density,xi) for xi in x)
    results = pool.map(sample_dpgmm, sample_args)
    return np.array([r for r in results])

def sample_dpgmm(args):
    (dpgmm,x) = args
    logPs = [prob.logProb([x]) for ind,prob in enumerate(dpgmm[1])]
    return logsumexp(logPs,b=dpgmm[0])
...
def merge_split_catalogs(realisations=None, base_path=None, 
    ...
    colors_dict = {
        ...
        "chunk7_samp": 'gray',
        "chunk8_samp": 'cyan',
        "chunk9_samp": 'limegreen',
        "chunk10_samp": 'mediumturquoise',
        "chunk11_samp": 'hotpink',
        "chunk12_samp": 'gainsboro',
        "chunk13_samp": 'dodgerblue',
        "chunk14_samp": 'slateblue',
        "chunk15_samp": 'rebeccapurple',
        "chunk16_samp": 'fuchsia',
        "chunk17_samp": 'crimson',
        "chunk18_samp": 'springgreen',
        "chunk19_samp": 'aqua',
        "chunk20_samp": 'powderblue',
        "combined": 'black'
                }
    ...
    joint_folder = os.path.join(base_path, f"{runtag}_split_data_merged")
    ...
    for k in realisations:
        ...
        out_folder = os.path.join(joint_folder, f"Realisation_Seed7_{runtag}{k}_{chunks}_split_data_merged")
        ...
        chunk_paths = {f"chunk{i}_samp": os.path.join(base_path, 
                    f"Realisation_Seed7_{runtag}{k}_{chunks}_{i}", 
        ...
        for path_name, path in chunk_paths.items():                      
            ...
            if model == "LambdaCDM":
                p1 = posteriors['h']
                # p2 = np.zeros(len(posteriors['h']))# posteriors['om']
                all_events[f"{path_name}"] = dict(p1=p1) # dict(p1=p1, p2=p2)
            elif model == "DE":
                p1 = posteriors['w0']
                p2 = posteriors['w1']
                ...
                p1 = posteriors['Xi0']
                all_events[f"{path_name}"] = dict(p1=p1)


        # joint_posterior = np.zeros((Nbins, Nbins), dtype=np.float64)
        pdf_p1 = np.zeros(Nbins, dtype=np.float64)
        ...
        if model == "LambdaCDM":
            ...
            # y_flat = np.linspace(0.314999, 0.315001, Nbins) # [0.04, 0.5]
        elif model == "DE":
        ...
        #     else:
        #         xlimits = [0.65, 0.8]
        #     ylimits = [0.314999, 0.315001] # 0.04, 0.50
        ...
        pdf_p1 = renormalise_1D(pdf_p1, dx)
...
base_path = '/sps/lisaf/cliu/results/EMRI_M1/3-sigma_zcut10_WKLS_SNR100/Error_boxes_'
model = 'LambdaCDM'
runtag = 'Normalised_Weight_'#'Normalised_Weight_'
realisations = ["1"] #["4","5","6","7","8","9","10"]
chunks = 2
chunk_realis_name = ''
nBins = 1024
source = 'dark_siren'
...
if __name__=="__main__":
    """First merge each single realisation with this script.
    For each realisation:
        Read the different chunks
        DPGMM them and create a joint folder with samples and plots
        Sample from the joint distribution
    Then run average_posteriors_single_source.py
    Finally run matrix_plots.py (to be implemented)
    """


    cosmo3G = 1
    if cosmo3G and "MBHB" not in source:
        truths = {'h': 0.673, 'om': 0.315, 'ol': 0.685,
                'w0': -1.0, 'wa': 0.0}
    else:
        truths = {'h': 0.73, 'om': 0.25, 'ol': 0.75,
                'w0': -1.0, 'wa': 0.0}
    ############
    ...
    merge_split_catalogs(realisations=realisations, 
                            base_path=base_path, 
                            runtag=runtag, chunks=chunks, 
                            chunk_realis_name=chunk_realis_name, 
                            model=model, Nbins=nBins)
